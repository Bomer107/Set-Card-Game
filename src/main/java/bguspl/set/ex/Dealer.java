...
public class Dealer implements Runnable {
    ...
    public void terminate() {
        for(int playerId = players.length - 1; playerId > -1; --playerId)
        {
            ...
            synchronized(player){
                System.out.println("dealer notify :"+player.id);
                player.notify();
            }
            player.terminate();
        }
    ...
    private void removeCardsFromTable() {
        if(playerCardsToRemove != null){
            table.removeCards(playerCardsToRemove,this);
            hint=true;
        }
    ...
    private void checkSet(Queue<Player> queueOfPlayers){
        ...
        if(queueOfPlayers.isEmpty()){
            ...
            if(!queueOfPlayers.isEmpty()){
                ischeck=true;
                System.out.println("dealer check queu" );
                env.logger.info("dealer checks cards");
                Player player = queueOfPlayers.poll();
                System.out.println(" check card of :"+player.id);
                table.sem2.release();
                System.out.println("sem2 is realese by  dealer"); 
                if(env.util.testSet(player.getCards())){
                    System.out.println("player "+player.id+"  is right");
                    right=player;
                    player.point();
                    ...
                    System.out.println("player, "+player.id+" ,penelty");
                    player.penalty();
                
                synchronized(player){
                    player.setDontGetInput(false);
                    System.out.println(" dontget= "+ player.getDontGetInput()); 
                    System.out.println(player.id+" notify by  dealer");
                    player.notify();
                } 
            }
        }
